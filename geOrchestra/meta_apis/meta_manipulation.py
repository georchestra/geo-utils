...
class Meta_manipulation:
    ...
    def add_thesaurus(metadata, title, server, thesaurus_value, local_thesaurus_name):

        catalog_specific_tags = (
            ...
            + server
            + """/geonetwork/srv/api/registries/vocabularies/local.theme."""
            + local_thesaurus_name
            + """">
                                            geonetwork.thesaurus.local.theme."""
            + local_thesaurus_name
            + """
                                        </gmx:Anchor>
                                    </gmd:code>
        ...
        modified_meta, status = re.subn(
            r"<\/gmd:descriptiveKeywords>([\w\W]+)<gmd:resourceConstraints>",
            r"</gmd:descriptiveKeywords>\1"
            + catalog_specific_tags
    ...
    def transform_metadata_from_geonode_to_gn(
        self,
        original_metadata,
        uuid,
        title,
                    ...
                    <gmd:MD_DigitalTransferOptions>
                        <gmd:onLine>
                            <gmd:CI_OnlineResource>
                                <gmd:linkage>
                                    <gmd:URL>https://"""
                + self.final_new_server
                + """/geoserver/ows</gmd:URL>
                                </gmd:linkage>
                                <gmd:protocol>
        ...
        modified_meta = re.sub(
            r'<gmd:edition gco:nilReason="missing">(\n.*)+<\/gmd:edition>',
            mdidentifier,
            modified_meta_tmp,
            re.M,
        )
        ...
        modified_meta_tmp = modified_meta
        modified_meta = re.sub(
            r"geonode:", catalog_name.lower() + ":", modified_meta_tmp
        )

        modified_meta_tmp = modified_meta
        modified_meta = re.sub(
            self.geonode_dns, self.final_new_server, modified_meta_tmp
        )

        # switch back thumb for later update
        modified_meta_tmp = modified_meta
        modified_meta = re.sub(
            r"{}\/uploaded\/thumbs".format(self.final_new_server),
            self.server + "/uploaded/thumbs",
            modified_meta_tmp,
        )

        modified_meta_tmp = modified_meta
        modified_meta = self.add_thesaurus(
            metadata=modified_meta_tmp,
            title="Projet " + catalog_name,
            server=final_new_server,
            thesaurus_value=catalog_name,
            local_thesaurus_name="projets",
        )
        modified_meta_tmp = modified_meta
        ...
        <gmd:contentInfo>
            <gmd:MD_FeatureCatalogueDescription>
                ...
                + """\"
                                              ...
                                              xlink:href="https://"""
                + self.final_new_server
                + """/geonetwork/srv/fre/csw?service=CSW&amp;request=GetRecordById&amp;version=2.0.2&amp;outputSchema=http://www.isotc211.org/2005/gmd&amp;elementSetName=full&amp;id="""
                + feat_uuid
                + """\"/>
            </gmd:MD_FeatureCatalogueDescription>
        </gmd:contentInfo>"""
    ...
    def transform_feature_from_geonode_to_gn(
        self, meta_feat, title, description, catalog_name, list_feat_detail=[]
    ):
        ...
        modified_feat = re.sub(
            r"<gmx:name>(\n.*)+<\/gmx:name>", new_title, meta_feat, re.M
        )
        new_description = (
            """<gmx:scope>
        <gco:CharacterString>"""
        ...
        modified_feat = re.sub(
            r"<gmx:scope>(\n.*)+<\/gmx:scope>", new_description, meta_feat, re.M
        )
        ...
        meta_feat = modified_feat
        modified_feat = re.sub(
            '<gfc:featureType xlink:href="#FT-' + catalog_name.lower() + ":(.*)\/>",
            "",
            meta_feat,
        )
...
if __name__ == "__main__":
    ...
    all_file_updated = Meta_manipulation.add_thesaurus(
        metadata=all_file,
        title="DonnéeType",
